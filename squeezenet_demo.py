...
def squeezenet_generate_random_batch(x, y, batch_size):
    datagen = ImageDataGenerator(
                rescale=1./255,
                shear_range=0.2,
                zoom_range=0.2,
                horizontal_flip=True)
    generator = datagen.flow(x, y=y, batch_size=batch_size)
#    size_data = x.shape[0]
...
def main():
    ...
    squeeze_model = squeeze_net.SqueezeNet_Model()
    model = squeeze_model.build_classifier_model()
    model.compile(
        optimizer=sgd, loss='categorical_crossentropy',
        metrics=['accuracy'])
    train_generator = squeezenet_generate_random_batch
...
#    dataset_cats = imagenet_cats.Imagenet_cats()
#    print("predicting")
#    results = model.predict(dataset_cats.x_test, batch_size=100, verbose=1)
#    print(results)
...
#        dataset.split_data_cross_validation([], "_all_data_in_train")
    ...
    if args.order in ["vgg16", "vgg19", "inception", "xception", "resnet"]:
    ...
    elif args.experiment == "schedule_imagenet":
