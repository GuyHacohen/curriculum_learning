...
def data_function_from_input(curriculum, curriculum_scheduler, batch_size,
    ...
    if curriculum == "None" or curriculum == "vanilla":
        data_function = train_keras_model.no_curriculum_data_function
    elif curriculum in ["curriculum", "vanilla", "anti"]:
...
def gradients_wrt_loss_dist(model, x, y_labels):
    num_layers = 3 # gradients only for the last 3 layers
    num_examples = x.shape[0]
    weights = model.trainable_weights # weight tensors
    weights = weights[-num_layers:]  
...
#        print(len(grads))
    ...
    dists_vars = [np.var([dist_between(grad, mean_grad) for grad in grads])
...
if __name__ == "__main__":
    ...
    if args.dataset.startswith('cifar100_subset'):
        ...
        if smaller_data:
            ...
            dataset = stl10.Stl10(normalize=False)
        model_lib = stl10_cifarlike_model.Stl10_Cifarlike_Model()
...
#        dataset.split_data_cross_validation([], "_all_data_in_train")
    ...
    if args.experiment == "train":
        ...
        start_time_all = time.time()
        histories =[]
        grad_vars_0 = np.array([0.0, 0.0, 0.0])
        grad_vars_1 = np.array([0.0, 0.0, 0.0])
        grad_vars_easy = np.array([0.0, 0.0, 0.0])
        grad_vars_rand = np.array([0.0, 0.0, 0.0])
        grads_easy = defaultdict(lambda: np.array([0.0, 0.0, 0.0]))  ## batch_num -> grads
        grads_all = defaultdict(lambda: np.array([0.0, 0.0, 0.0]))  ## batch_num -> grads
        for repeat in range(args.repeats):
            data_function = data_function_from_input(args.curriculum, args.curriculum_scheduler, args.batch_size,
                                             dataset, order, args.batch_increase,
                                             args.increase_amount, args.starting_percent,
            ...
            random_idxes = rand_order[:300]
            x_easy = dataset.x_train[easy_idxes, :, :, :]
            x_rand = dataset.x_train[random_idxes, :, :, :]
            y_easy = dataset.y_train_labels[easy_idxes, :]
            y_rand = dataset.y_train_labels[random_idxes, :]
            grads_all[0] += gradients_wrt_loss_dist(model, x_rand, y_rand)
            grads_easy[0] += gradients_wrt_loss_dist(model, x_easy, y_easy)
            grads_each = 100
            for cur_batch_num in range(grads_each, num_batchs + grads_each, grads_each):
                
                history = train_keras_model.train_model_batches(model, dataset.x_train, dataset.y_train_labels, dataset.x_test,
                                                                dataset.y_test_labels, grads_each, verbose=args.verbose,
                                                                batch_size=args.batch_size,
                                                                test_each=args.test_each,
                                                                initial_lr=args.learning_rate,
                                                                lr_scheduler=lr_scheduler, loss='categorical_crossentropy',
                ...
                grads_all[cur_batch_num] += gradients_wrt_loss_dist(model, x_rand, y_rand)
                grads_easy[cur_batch_num] += gradients_wrt_loss_dist(model, x_easy, y_easy)
                print("finished batch num:", cur_batch_num)
        ...
        grads_easy = {k: v/args.repeats for k,v in grads_easy.items()}
        grads_all = {k: v/args.repeats for k,v in grads_all.items()}
        with open(output_path + "_grads_angles", 'wb') as file_pi:
            pickle.dump((grads_easy, grads_all), file_pi)
